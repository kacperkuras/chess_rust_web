...
use crate::auth::jwt;
use crate::db;
use crate::game::game::{Game, GameStatus, Player, SharedGameState};
...
impl Actor for ChessSession {
    ...
    fn started(&mut self, ctx: &mut Self::Context) {
        ...
        actix::spawn(async move {
            ...
            if let Some(opponent) = state.queue.pop() {
                let game_id = match db::games::create_game(
                    &db_pool,
                    opponent.user_id,
                    new_player.user_id,
                    opponent.elo,
    ...
    fn stopped(&mut self, _: &mut Self::Context) {
            ...
            .position(|g| g.white.user_id == self.user_id || g.black.user_id == self.user_id)
        {
            ...
            actix::spawn(async move {
                if let Err(e) = db::games::end_game(&db_pool, game_id, &result).await {
                    eprintln!("Błąd kończenia gry w DB: {:?}", e);
                }

                if let Err(e) =
                    db::statistics::update_users_statistics(&db_pool, white_player_id, black_player_id, &result)
                        .await
                {
                ...
                match parsed.unwrap() {
                                    ...
                                    if game.status != GameStatus::InProgress {
                                        ...
                                        wrap_future({
                                                async move {
                                                    ...
                                                    if let Err(e) = db::statistics::update_users_statistics(&db_pool, white_player_id, black_player_id, &result).await {
                                                        eprintln!("Błąd aktualizacji statystyk użytkowników: {:?}", e);
                                                    }
                                                    Ok::<(), ()>(())
                                                }
                    ...
                    ClientMessage::Chat { message } => {
                        ...
                        if let Some(game) = state.games.iter().find(|g| {
                            ...
                            ctx.spawn(
                                wrap_future(async move {
                                    db::chat_messages::add_chat_message(&db_pool, game_id.unwrap(), sender_id, &chat_message)
                                        .await
                                })
...
async fn websocket_handler(req: HttpRequest, path: web::Path<String>, stream: web::Payload, db_pool: web::Data<MySqlPool>, data: web::Data<SharedGameState>) -> actix_web::Result<HttpResponse> {
    ...
    let user_stats_result = db::statistics::get_user_statistics(&db_pool, user_id).await;
    let user = db::users::get_user_by_id(&db_pool, user_id).await;
